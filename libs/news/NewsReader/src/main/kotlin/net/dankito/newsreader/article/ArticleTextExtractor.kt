...
object ArticleTextExtractor {
    ...
    fun extractContent(doc: Document?, contentIndicator: String?): String? {
        ...
        for (entry in metas) {
            if (entry.hasAttr("property") && "og:image" == entry.attr("property")) {
    ...
    private fun removeStyles(doc: Document): Document {
    ...
    private fun removeAds(doc: Document) {
    ...
    private fun getMetas(doc: Document): Collection<Element> {
        ...
        for (el in doc.select("head").select("meta")) {
            nodes.add(el)
        }
        return nodes
    }
